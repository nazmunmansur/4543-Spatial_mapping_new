...
def dump_yaml(meta: dict) -> str:
    order = [
        "schema_version", "title", "type", "status",
        "due", "points", "notebooks", "files",
...
# def walk_candidate_folders(assignments_root: Path, section_arg: str | None, verbose: bool) -> List[Path]:
...
def walk_candidate_folders(assignments_root: Path, section_arg: str | None, verbose: bool) -> List[Path]:
    ...
    for sec in sections:
        for p in sorted(sec.rglob("*")):
            ...
            # ✅ Option A: ignore legacy notebooks container entirely
            if p.name.lower() == "notebooks":
                if verbose:
                    print(f"[skip] legacy container: {p}")
                continue

            # ✅ Also skip anything *inside* a legacy notebooks container
            if any(parent.name.lower() == "notebooks" for parent in p.parents):
                continue
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="generate_meta", description="Generate/refresh meta.yaml under Assignments/.")
    ...
    ap.add_argument("--dry-run", action="store_true", help="Show actions, do not write.")
    ap.add_argument("--force", action="store_true", help="Overwrite existing meta.yaml files.")
    ap.add_argument("--refresh", action="store_true", help="Refresh notebooks/files lists (preserves title/due/points/status).")
    ap.add_argument("--prune", action="store_true", help="Rewrite notebooks/files keys even if they existed (cleanup).")
    ap.add_argument("--verbose", action="store_true", help="Print discovered candidates.")
    ap.add_argument("--show-existing", action="store_true", help="Print meta.yaml paths that are kept (not overwritten).")

    ap.add_argument("--create", action="store_true", help="Create meta.yaml for a specific folder only.")
...
def main() -> int:
    ...
    folders = walk_candidate_folders(assignments_root, args.section, verbose=args.verbose)
    if not folders:
